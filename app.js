...
const POLICIES={
  medical:{kind:'medical',coll:'medPolicies',label:'Medical Insurance',short:'Medical Policy',icon:'🏥',fields:MED_FIELDS,prefix:'MED',premiumKey:'annualPremium',cols:['plan','coverageClass']},
  life:{kind:'life',coll:'lifePolicies',label:'Life Insurance',short:'Life Policy',icon:'🛡️',fields:LIFE_FIELDS,prefix:'LIF',premiumKey:'premium',cols:['sumAssured','beneficiary']}
};
...
function seedAll(){
  ...
  // employees
  const E=(o)=>mk('employees',Object.assign({status:'Active',nationality:'Saudi',company:co.id},o));
  const e1=E({empNo:'E-1001',name:'Tariq Al-Otaibi',iqama:'2412345671',iqamaExpiry:d(120),passport:'A1234567',dob:'1985-03-12',gender:'Male',nationality:'Saudi',businessUnit:bu1.id,division:divFleet.id,department:depOps.id,jobTitle:'Operations Manager',mobile:'+966 50 111 2222',email:'tariq@naghi.com'});
  ...
  const e4=E({empNo:'E-1004',name:'Yousef Al-Mutairi',iqama:'2412345674',iqamaExpiry:d(300),passport:'C5566778',dob:'1982-01-19',gender:'Male',nationality:'Saudi',businessUnit:bu1.id,division:divFin.id,department:depAcc.id,jobTitle:'Finance Manager',mobile:'+966 55 222 3333',email:'yousef@naghi.com'});
  ...
  const e6=E({empNo:'E-1006',name:'Abdullah Al-Shehri',iqama:'2412345676',iqamaExpiry:d(210),passport:'D1122334',dob:'1979-09-08',gender:'Male',nationality:'Saudi',businessUnit:bu3.id,division:divFleet.id,department:depOps.id,jobTitle:'Warehouse Lead',mobile:'+966 53 444 5555',email:'abdullah@naghi.com'});
...
function nav(page,arg){
  ...
  const fn={dashboard:renderDashboard,notifications:renderNotifications,employees:renderEmployees,dependents:renderDependents,
    medical:()=>renderPolicies('medical'),life:()=>renderPolicies('life'),documents:renderDocuments,reports:renderReports,
...
function drawEmployees(){
  ...
  body.innerHTML=rows.map(e=>{
    ...
    const pol=DB.all('medPolicies').filter(p=>p.employeeId===e.id).length+DB.all('lifePolicies').filter(p=>p.employeeId===e.id).length;
...
function renderEmployeeDetail(id,tab){
  ...
  document.getElementById('main').innerHTML=`
    ...
    <div class="actions">
      ${can('policy.create')?`<button class="btn btn-ghost" onclick="policyForm('medical',null,'${e.id}')">+ Medical</button><button class="btn btn-ghost" onclick="policyForm('life',null,'${e.id}')">+ Life</button>`:''}
...
function empPolicies(e){
  ...
  return tbl(POLICIES.medical,med)+tbl(POLICIES.life,life);
}
...
function exportDependents(){
  const rows=DB.all('dependents').map(d=>({'Dependent No':d.depNo,'Employee':empName(d.employeeId),'Relationship':d.relationship,'Dependent Name':d.name,'Iqama / National ID':d.iqama,'Iqama Expiry':d.iqamaExpiry,'Passport':d.passport,'Date of Birth':d.dob,'Gender':d.gender,'Status':d.status}));
...
function renderPolicies(kind){
  ...
  const extraCols=cfg.kind==='medical'?'<th>Plan</th><th>Coverage</th>':'<th class="right">Sum Assured</th><th>Beneficiary</th>';
  document.getElementById('main').innerHTML=`
    <div class="page-head"><div><div class="crumb">Insurance</div><h2>${cfg.icon} ${esc(cfg.label)}</h2></div>
      <div class="actions">
        ...
        ${can('policy.create')?`<button class="btn" onclick="policyForm('${kind}')">+ New ${cfg.kind==='medical'?'Medical':'Life'} Policy</button>`:''}
      </div></div>
    <div class="toolbar">
    ...
    <div class="card"><div class="table-wrap"><table>
      <thead><tr><th>Policy No.</th><th>Employee</th><th>Insurer</th>${extraCols}<th>Expiry</th><th class="right">Premium</th><th>Status</th><th class="right">Actions</th></tr></thead>
...
function drawPolicies(kind){
  ...
  if(!list.length){body.innerHTML=`<tr><td colspan="9" class="empty">No policies match.</td></tr>`;return;}
  body.innerHTML=list.map(p=>{
    ...
    const extra=cfg.kind==='medical'?`<td>${esc(p.plan||'—')}</td><td>${esc(p.coverageClass||'—')}</td>`:`<td class="right">${num(p.sumAssured)}</td><td>${esc(p.beneficiary||'—')}</td>`;
...
function policyForm(kind,id,presetEmp){
  ...
  modal((id?(ro?'View ':'Edit '):'New ')+cfg.short+(id?` — ${esc(rec.policyNo)}`:''),
    `<div class="form-grid">${cfg.fields.map(f=>`<div class="field ${f.type==='textarea'?'full':''}"><label>${esc(f.label)} ${reqStar(f)}</label>${fieldInput(f,rec[f.k],ro)}</div>`).join('')}</div>
     <p class="help" style="margin-top:8px;color:var(--muted);font-size:11.5px">Policy Number must be unique. Only <b>active</b> employees can receive new policies.</p>`,
    ro?[{label:'Close',cls:'btn-ghost',fn:closeModal}]:[
      {label:'Cancel',cls:'btn-ghost',fn:closeModal},
      {label:id?'Save Changes':'Create',cls:'btn',fn:()=>savePolicy(kind,id)}
    ],true);
}
function savePolicy(kind,id){
  ...
  const emp=DB.byId('employees',patch.employeeId);
  if(!id&&(!emp||emp.status!=='Active')){toast('Only active employees can receive new policies.','err');return;}
...
function renderPolicyDetail(kind,id,tab){
  ...
  document.getElementById('main').innerHTML=`
    ...
    <div class="tabs">
      ${['details','documents','history'].map(t=>`<div class="tab ${polTab===t?'active':''}" onclick="renderPolicyDetail('${kind}','${id}','${t}')">${({details:'Details',documents:'Documents',history:'Change History'})[t]}</div>`).join('')}
    </div>
    <div id="pdBody"></div>`;
...
function policyDetails(cfg,p){
  ...
  return `<div class="grid2">
    ...
    <div class="card"><div class="card-h">Employee</div><div class="card-b">
      ${(()=>{const e=DB.byId('employees',p.employeeId);if(!e)return '<div class="empty">Employee not found.</div>';
        return kv('Name',esc(e.name))+kv('Employee No.',esc(e.empNo))+kv('Iqama / ID',esc(e.iqama))+kv('Division',esc(refLabel('divisions',e.division)))+kv('Business Unit',esc(refLabel('businessUnits',e.businessUnit)))+kv('Status',e.status)+`<div style="margin-top:10px"><a href="#" onclick="renderEmployeeDetail('${e.id}');return false" class="btn btn-sm btn-ghost">Open employee →</a></div>`;})()}
    </div></div>
  </div>`;
}
...
function renderDocuments(){
  ...
  document.getElementById('main').innerHTML=`
    ...
    <div class="kpi-grid">
      ...
      <div class="kpi muted"><div class="v">${DB.all('medPolicies').concat(DB.all('lifePolicies')).filter(p=>!(p.attachments||[]).length&&policyEff(p)==='Active').length}</div><div class="l">Active Policies w/o Docs</div></div>
    </div>
    <div class="card"><div class="card-h">Document Register</div><div class="table-wrap"><table>
      ...
      <tbody>${all.length?all.map(({owner,ownerType,coll,id,d,i})=>{const exp=d.expiry?daysUntil(d.expiry):null;return `<tr>
        ...
        <td><a href="#" onclick="${coll==='employees'?`renderEmployeeDetail('${id}','documents')`:`renderPolicyDetail('${coll==='medPolicies'?'medical':'life'}','${id}','documents')`};return false">${esc(owner)}</a></td>
...
function computeNotifications(){
  ...
  const policyAlerts=(kind)=>{const cfg=POLICIES[kind];DB.all(cfg.coll).forEach(p=>{const es=policyEff(p);
    if(es==='Active'){const d=daysUntil(p.expiryDate);
      if(d!=null&&d>=0&&d<=maxR)out.push({icon:cfg.icon,sev:d<=7?'high':d<=30?'med':'low',title:`${cfg.short} ${p.policyNo} expires in ${d} day(s)`,sub:empName(p.employeeId),kind,pid:p.id});
      if(!(p.attachments||[]).length)out.push({icon:'📄',sev:'low',title:`Missing documents — ${p.policyNo}`,sub:`${cfg.short} · ${empName(p.employeeId)}`,kind,pid:p.id});}
    if(es==='Expired')out.push({icon:cfg.icon,sev:'high',title:`${cfg.short} expired — ${p.policyNo}`,sub:`${empName(p.employeeId)} · ${p.expiryDate}`,kind,pid:p.id});
  });};
  policyAlerts('medical');policyAlerts('life');
...
function renderDashboard(){
  ...
  const med=DB.all('medPolicies');const life=DB.all('lifePolicies');
  const allPol=med.map(p=>({p,cfg:POLICIES.medical})).concat(life.map(p=>({p,cfg:POLICIES.life})));
  ...
  const annualCost=activeMed.reduce((s,p)=>s+Number(p.annualPremium||0),0)+activeLife.reduce((s,p)=>s+Number(p.premium||0),0);
  const companyCost=activeMed.reduce((s,p)=>s+Number(p.companyContribution||0),0);
  // cost by division
  const byDiv={};const byInsurer={};
  activeMed.forEach(p=>{const k=refLabel('divisions',empDivision(p.employeeId))||'—';byDiv[k]=(byDiv[k]||0)+Number(p.annualPremium||0);const ins=refLabel('insuranceCompanies',p.insurer)||'—';byInsurer[ins]=(byInsurer[ins]||0)+Number(p.annualPremium||0);});
  activeLife.forEach(p=>{const k=refLabel('divisions',empDivision(p.employeeId))||'—';byDiv[k]=(byDiv[k]||0)+Number(p.premium||0);const ins=refLabel('insuranceCompanies',p.insurer)||'—';byInsurer[ins]=(byInsurer[ins]||0)+Number(p.premium||0);});
...
const REPORTS=[
  ...
  {key:'activePolicies',name:'Active Policies',build:()=>{const m=DB.all('medPolicies').filter(p=>policyEff(p)==='Active').map(p=>({'Type':'Medical','Policy No':p.policyNo,'Employee':empName(p.employeeId),'Insurer':refLabel('insuranceCompanies',p.insurer),'Effective':p.effectiveDate,'Expiry':p.expiryDate,'Premium SAR':Number(p.annualPremium||0)}));const l=DB.all('lifePolicies').filter(p=>policyEff(p)==='Active').map(p=>({'Type':'Life','Policy No':p.policyNo,'Employee':empName(p.employeeId),'Insurer':refLabel('insuranceCompanies',p.insurer),'Effective':p.effectiveDate,'Expiry':p.expiryDate,'Premium SAR':Number(p.premium||0)}));return m.concat(l);}},
  {key:'expiredPolicies',name:'Expired Policies',build:()=>{const m=DB.all('medPolicies').filter(p=>policyEff(p)==='Expired').map(p=>({'Type':'Medical','Policy No':p.policyNo,'Employee':empName(p.employeeId),'Insurer':refLabel('insuranceCompanies',p.insurer),'Expired On':p.expiryDate}));const l=DB.all('lifePolicies').filter(p=>policyEff(p)==='Expired').map(p=>({'Type':'Life','Policy No':p.policyNo,'Employee':empName(p.employeeId),'Insurer':refLabel('insuranceCompanies',p.insurer),'Expired On':p.expiryDate}));return m.concat(l);}},
  {key:'renewals',name:'Renewal Report (90d)',build:()=>{const out=[];['medical','life'].forEach(kind=>{const cfg=POLICIES[kind];DB.all(cfg.coll).forEach(p=>{const d=daysUntil(p.expiryDate);if(policyEff(p)==='Active'&&d>=0&&d<=90)out.push({'Type':cfg.kind==='medical'?'Medical':'Life','Policy No':p.policyNo,'Employee':empName(p.employeeId),'Insurer':refLabel('insuranceCompanies',p.insurer),'Expiry':p.expiryDate,'Days Left':d,'Premium SAR':premiumOf(cfg,p)});});});return out.sort((a,b)=>a['Days Left']-b['Days Left']);}},
  {key:'costDivision',name:'Insurance Cost by Division',build:()=>{const o={};const add=(p,prem)=>{const k=refLabel('divisions',empDivision(p.employeeId))||'—';o[k]=o[k]||{Division:k,Policies:0,'Annual Cost SAR':0};o[k].Policies++;o[k]['Annual Cost SAR']+=prem;};DB.all('medPolicies').filter(p=>policyEff(p)==='Active').forEach(p=>add(p,Number(p.annualPremium||0)));DB.all('lifePolicies').filter(p=>policyEff(p)==='Active').forEach(p=>add(p,Number(p.premium||0)));return Object.values(o);}},
  {key:'costBU',name:'Insurance Cost by Business Unit',build:()=>{const o={};const add=(p,prem)=>{const k=refLabel('businessUnits',empBusinessUnit(p.employeeId))||'—';o[k]=o[k]||{'Business Unit':k,Policies:0,'Annual Cost SAR':0};o[k].Policies++;o[k]['Annual Cost SAR']+=prem;};DB.all('medPolicies').filter(p=>policyEff(p)==='Active').forEach(p=>add(p,Number(p.annualPremium||0)));DB.all('lifePolicies').filter(p=>policyEff(p)==='Active').forEach(p=>add(p,Number(p.premium||0)));return Object.values(o);}},
  {key:'costCompany',name:'Insurance Cost by Company',build:()=>{const o={};const add=(p,prem)=>{const k=refLabel('companies',empCompany(p.employeeId))||'—';o[k]=o[k]||{Company:k,Policies:0,'Annual Cost SAR':0};o[k].Policies++;o[k]['Annual Cost SAR']+=prem;};DB.all('medPolicies').filter(p=>policyEff(p)==='Active').forEach(p=>add(p,Number(p.annualPremium||0)));DB.all('lifePolicies').filter(p=>policyEff(p)==='Active').forEach(p=>add(p,Number(p.premium||0)));return Object.values(o);}},
  {key:'costInsurer',name:'Cost by Insurance Company',build:()=>{const o={};const add=(p,prem)=>{const k=refLabel('insuranceCompanies',p.insurer)||'—';o[k]=o[k]||{Insurer:k,Policies:0,'Annual Cost SAR':0};o[k].Policies++;o[k]['Annual Cost SAR']+=prem;};DB.all('medPolicies').filter(p=>policyEff(p)==='Active').forEach(p=>add(p,Number(p.annualPremium||0)));DB.all('lifePolicies').filter(p=>policyEff(p)==='Active').forEach(p=>add(p,Number(p.premium||0)));return Object.values(o);}},
...
/* ============================================================
   ...
   ============================================================ */
seedAll();
const _sess=localStorage.getItem('mlims_session');
if(_sess){try{SESSION=JSON.parse(_sess);startApp();}catch(e){}}
